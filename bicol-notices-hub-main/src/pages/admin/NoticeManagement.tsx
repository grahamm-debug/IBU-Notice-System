...
const NoticeManagement = () => {
  ...
  const filteredNotices = notices.filter((notice) => {
    ...
    const matchesDepartment =
      ...
      notice.target_departments?.includes(departmentFilter);
                ...
                return (
                  <TableRow
                    ...
                    className={cn(
                      ...
                      notice.priority === 'critical' && 'bg-destructive/5'
                    )}
                  >
                    <TableCell>
                      <Checkbox
                    ...
                    <TableCell>
                      <div className="flex flex-wrap gap-1">
                        {notice.target_departments?.slice(0, 2).map((deptId) => (
                          <Badge key={deptId} variant="secondary" className="text-xs">
                            {getDepartmentName(deptId)}
                          </Badge>
                        ))}
                        {(notice.target_departments?.length || 0) > 2 && (
