...
const AnalyticsPage = () => {
  ...
  const fetchAnalytics = async () => {
    try {
      ...
      // Fetch notice reads
      const { data: reads } = await supabase.from('notice_reads').select('*');
      ...
      if (notices && profiles) {
        ...
        const draft = notices.filter((n) => !n.is_published).length;
        const totalReads = reads?.length || 0;
        const readRate = published > 0 ? Math.round((totalReads / (published * profiles.length)) * 100) : 0;
